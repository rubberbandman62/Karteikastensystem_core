...
import com.iks.hto.karteikastensystem.core.internal.Activator;
...
public class XMIKarteikastenSystemResource implements
		IKarteikastenSystemResource {
	...
	private EditingDomain editingDomain = new EditingDomain() {
		...
		public CommandStack getCommandStack() {
			return new CommandStack() {
				...
				public void removeCommandStackListener(
						CommandStackListener listener) {
					wrappedEditingDomain.getCommandStack()
				...
				public Command getUndoCommand() {
					return wrappedEditingDomain.getCommandStack()
							.getUndoCommand();
				}
				...
				public void addCommandStackListener(
						CommandStackListener listener) {
					wrappedEditingDomain.getCommandStack()
							.addCommandStackListener(listener);
				}
			};
		}
		...
		public Command createCommand(Class<? extends Command> commandClass,
			...
			return wrappedEditingDomain.createCommand(commandClass,
					commandParameter);
		}
	};
	...
	private List<IKarteikastenSystemResource.Listener> listeners = new ArrayList<Listener>();

	public XMIKarteikastenSystemResource(String uriString) {
		...
		wrappedEditingDomain = new AdapterFactoryEditingDomain(adapterFactory,
		...
		System.out
				.println("XMIKarteikastenSystemResource -> try to load from: "
						+ uriString);
	...
	public KarteikastenSystem getKarteikastenSystem() {
		if (resource.getContents().size() == 0) {
			resource.getContents().add(
					KarteikastensystemFactory.eINSTANCE
	...
	private void setModified(boolean modified) {
		if (this.modified != modified) {
			...
			for (IKarteikastenSystemResource.Listener listener : listeners) {
	...
	public IStatus canRedo() {
		return getEditingDomain().getCommandStack().canRedo() ? Status.OK_STATUS
				: Status.CANCEL_STATUS;
	}

	public IStatus canUndo() {
		return getEditingDomain().getCommandStack().canUndo() ? Status.OK_STATUS
				: Status.CANCEL_STATUS;
	}
}
